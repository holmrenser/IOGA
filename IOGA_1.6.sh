...
#Here, folders for the unzipped forward (READ_1) and reverse (READ_2) reads are created.
...
#Below, the loop starts. NEWSIZE below counts the number of reads that are maintained each round for round i. 
...
#Here, Bowtie2 starts. An index is created, the reads are mapped and subsequently filtered out and saved to a mappedreads file. 
	...
	samtools view -Sh -F 4 $FILENAME.$i.sam | sort -k 3,3 -k 4,4n > mappedreads.$FILENAME.$i.sam
	
	#printf "SAM to sorted BAM\n"
	#samtools view -bS mappedreads.$FILENAME.$i.sam | samtools sort - mappedreads_sorted.$FILENAME.$i.sam	
	#printf "Sorted BAM to sorted SAM\n"
	#samtools view -h mappedreads_sorted.$FILENAME.$i.sam > mappedreads.$FILENAME.$i.sam
...
#Below we count the number of reads using 'wc' and then define that count (contained in the first #column) as NEWSIZE.
	...
	seqtk subseq $PREFIX/$FILENAME\_1.fastq $FILENAME.$i.names_1 > $FILENAME\_1_reads$i.fastq
	printf "Writing reverse reads\n"
	seqtk subseq $PREFIX/$FILENAME\_2.fastq $FILENAME.$i.names_2 > $FILENAME\_2_reads$i.fastq
...
if [ $lastround -gt 0 ]
	...
	Rscript --no-save /home/biosys/Applications/assemblyplotter.r
	...
	do
		...
		python /home/biosys/Applications/wiggleplotter.py $FILENAME.$o.coverage.wig		
